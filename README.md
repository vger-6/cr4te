...
> ⚠️ Note
> cr4te.json files are automatically generated by the build-json command and saved directly into each creator’s original folder (e.g., /path/to/Artists/*/cr4te.json). These files can be manually edited to override or extend metadata such as birth dates, aliases, tags, or project descriptions. They are preserved across HTML builds but will be regenerated (and overwritten) each time you re-run build-json, unless preserved manually.
