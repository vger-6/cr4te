window.cr4te.onReady(() => {
  const mobileBreakpoint = window.utils.getBreakpointPx('--layout-mobile-breakpoint') || 768;
  ...
  originalPlaceholders.forEach(original => {
    ...
    if (!mobile) {
      console.debug(`Missing .mobile-placeholder for target ${targetId}`);
      return;
    }
    const contents = Array.from(original.children);
    if (contents.length === 0) {
      console.debug(`No content inside .original-placeholder with target ${targetId}`);
      return;
    }
