...
- Render labels semibold, slightly smaller than values, and in a dedicated neutral theme color that is visually distinct from links.
...
- Leave tag-category labels and tag chips unchanged because they are a separate UI concept.
...
```css
.meta-label {
  color: var(--theme-meta-label-text);
  font-size: 0.875rem;
  font-weight: 600;
}
```
...
- Add a dedicated metadata-label text token that defaults to the existing muted text color and can be overridden independently by themes.
...
<dl class="meta-list info-block__meta">
  <div class="meta-entry">
    <dt class="meta-label">Release Date</dt>
...
6. Style labels with the dedicated neutral metadata-label token, a slightly smaller font size, and semibold weight while preserving value and link styling.
...
- Template regression test proving tag-category labels and tag links remain unchanged.
...
- Browser test proving metadata labels use the dedicated neutral theme token, remain distinct from links, and remain legible in every built-in theme.
...
- Do not change tag-category presentation, tag links, badges, breadcrumbs, section titles, search syntax, or search-index metadata.
