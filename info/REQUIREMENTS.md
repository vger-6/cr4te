...
- **SITE-016:** Creator and project detail pages must present metadata consistently as stacked label-and-value entries without label colons, including creator, collaboration, participant, and member information embedded in those pages. Portrait-oriented information blocks must place single-column metadata beside the image when sufficient width is available. Landscape-oriented information blocks must place metadata below the image in two equal-width columns when sufficient width is available. Narrow layouts must stack images and metadata and present metadata in one column without hiding content.
