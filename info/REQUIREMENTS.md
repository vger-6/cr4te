...
- **LIFE-005:** Portrait discovery and portrait rendering must be independently controlled by resolved configuration. Portrait discovery and portrait-role assignment during library indexing must depend only on `media_rules` and remain invariant across creator-overview display mode and detail-page image visibility settings. An omitted CLI portrait-discovery override must preserve its configured value, and portrait rendering must be controlled only by configuration.
...
- **ASSET-002:** Failed source-derived thumbnails that are configured to use generated fallbacks must use generated defaults after reporting the asset issue. Detail-page profile and cover image roles configured as `if_available` must omit the image after reporting the asset issue.
...
- **ASSET-010:** Cover discovery must search only below the corresponding project folder and fall back from named matches to the first landscape-oriented eligible image, then to the first eligible image. When no source image is selected, generated cover fallbacks are available to card contexts but detail-page cover roles must remain empty.
...
- **SITE-014:** The creator-overview text-card display mode must render creator overview entries without images or icon badges and must perform no creator-overview thumbnail work.
- **SITE-015:** Detail-page cover and profile-image visibility must be configurable per page role with `show`, `if_available`, and `hide` modes. `show` must render a source-derived thumbnail when possible and fall back to the generated default for the role when no usable source image exists. `if_available` must render only usable source-derived thumbnails and must not use generated defaults. `hide` must omit image markup and skip thumbnail work for that role.
- **SITE-016:** Creator and project detail pages must present metadata consistently as stacked label-and-value entries without label colons, including creator, collaboration, participant, and member information embedded in those pages. Detail-page creator and collaboration profile sections must use the configured Profile title. Member and participant profile blocks must be grouped under the configured plural creator label instead of each receiving an individual name title. Main creator portrait and project cover information blocks must place single-column metadata beside the image unless the rendered thumbnail is clearly landscape, defined as width divided by height greater than or equal to 1.3. Landscape and image-less information blocks must present metadata in two equal-width columns when sufficient width is available, with landscape metadata placed below the image and entries sharing a row remaining top-aligned. Embedded profile blocks with images, including project-page creator and collaboration profiles plus member and participant lists, must keep the side-by-side layout. Narrow layouts must stack images and metadata and present metadata in one column without hiding content.
...
- **SITE-018:** Tag category labels must use the same visual label presentation as metadata labels and omit trailing colons. Tag values on creator, project, and tag overview pages must render as informational chips with keyboard-operable action menus rather than direct links. Tag action menus must offer creator-overview and project-overview filter destinations, and creator-page menus must also offer a current-creator project filter when the tag is project-derived.
...
- **SITE-021:** Generated human-readable phrases that combine dynamic or configurable values must use validated complete named-placeholder label formats where wording or word order may vary. Collaboration project subtitles, overview search placeholders, site-logo accessible labels, and creator/project image descriptions must not be assembled from hardcoded sentence fragments.
...
- **SITE-024:** Generated navigation must clearly identify the current overview and expose a creator/project page's contextual breadcrumb without redundantly repeating page-title chrome. Contextual breadcrumbs must stay contained on one line, use ellipses when their labels overflow, expose full labels only for actually truncated items, and move below the primary navigation on narrow screens while the logo, primary navigation, and theme control remain together.
- **SITE-025:** Every generated page must place global navigation on a full-width theme-controlled surface and place its accessible level-one title inside the shared inset content region. Overview and detail pages must keep that title available to assistive technology while omitting the visible title strip; detail-page titles must instead surface as contextual breadcrumbs. Overview search controls and overview or detail panels must align with the shared content edges. Search controls must use body typography and retain usable single-line input behavior. Every theme must provide coherent contrast for navigation, search controls, and panels.
- **SITE-026:** Generated-site typography must keep content visually primary through a restrained hierarchy: page titles must be more prominent than section titles, section titles must be more prominent than body text, and Markdown headings must remain subordinate to their section while preserving readable, distinguishable levels. Structural and compact UI labels such as section titles, contextual breadcrumbs, tags, and audio-track labels must stay on one line with ellipses. Preview text such as card captions and summaries must wrap but clamp after two lines. Detail prose and normal metadata must wrap freely without ellipses, except that creator About and project Description prose may collapse after independently configured desktop and mobile line limits when the rendered content exceeds those limits. Collapsed prose must be expandable with configurable native-button labels. Generated documents must expose the configured rendering document language, defaulting to `en-US`, and expandable prose must allow language-aware hyphenation. Truncated or clamped text must expose full text only when actually clipped.
...
- **SITE-028:** Creator-overview card display mode must be independent from detail-page profile-image visibility. Text mode must use text cards on creator overviews while configured detail-page profile images may still render. Image mode must preserve creator portrait image cards and their icon badges.
