...
- **SITE-023:** Generated pages must render contextual empty states when major visible content regions would otherwise appear blank. Empty creator and project overviews must omit unusable search controls and show configured collection-empty messages. Overview searches with no matches must keep the search control visible and show a configured polite no-results state. Empty tag pages, creator right columns without projects, collaboration projects, or creator-level media, and project media columns without media must show configured empty-state messages. Absent optional sections must remain omitted instead of each receiving its own empty-state panel, and static empty states must not require JavaScript.
- **SITE-024:** Generated pages must present the current overview page in strong theme text and creator or project detail titles in the navigation instead of as separate level-one page headings. A vertical separator must distinguish contextual detail breadcrumbs from the primary navigation, while breadcrumb separators use their dedicated muted theme color. At narrow widths the logo, primary navigation, and theme control must remain on the first row while only the contextual breadcrumb moves to a second row without a leading separator.
