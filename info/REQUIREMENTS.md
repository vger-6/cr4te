...
- **SITE-025:** Every generated page must place global navigation on a full-width theme-controlled surface and place its full-width level-one title inside a shared inset content region. The title must remain visually unboxed, while overview search controls and overview or detail panels must align with the title's content edges. Search controls must use body typography and retain usable single-line input behavior. Every theme must provide coherent contrast for navigation, titles, search controls, and panels.
