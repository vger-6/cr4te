...
- **ASSET-002:** Failed source-derived thumbnails that are configured to use generated fallbacks must use generated defaults. Detail-page profile and cover image roles must omit the image after reporting the asset issue.
...
- **SITE-015:** Detail-page cover and profile-image visibility must be configurable per page role with `show` and `hide` modes. `hide` must omit image markup and skip thumbnail work for that role. `show` must render a source-derived thumbnail only when the source image exists and remains usable; missing or recoverably unreadable sources must leave the image role empty instead of using generated defaults.
