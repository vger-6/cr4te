...
class TemplateRendererTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for rendered in (creator_rendered, project_rendered, tags_rendered):
                with self.subTest():
                    ...
                    self.assertIn('class="tag-action-icon" aria-hidden="true"', rendered)
                    self.assertNotIn("tag-action-separator", rendered)
    ...
    def test_detail_header_renders_contextual_breadcrumb_titles(self):
        with tempfile.TemporaryDirectory() as tmp:
            ctx = context_for(Path(tmp) / "input", Path(tmp) / "site")
            rendered = env.get_template("partials/_page_header.html.j2").render(
                ...
                page_shell=PageShellContext(
                    ...
                    navigation_items=(
                        ...
                        NavigationItem("Displayed Landscapes", "", current=True, starts_section=True),
                    ),
                ),
            )
            ...
            creator_rendered = env.get_template("partials/_page_header.html.j2").render(
                ...
                page_shell=PageShellContext(
                    ...
                    navigation_items=(
                        ...
                        NavigationItem(ctx.site_labels.entity.projects, "../projects.html"),
                        NavigationItem(ctx.site_labels.entity.tags, "../tags.html"),
                        NavigationItem("Displayed Noomi", "", current=True, starts_section=True),
                    ),
                ),
            )
    ...
    def test_detail_templates_group_profile_sections_under_role_titles(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('class="info-block info-block--portrait"', creator_rendered)
            ...
            self.assertIn('class="info-block info-block--portrait"', project_rendered)
    ...
    def test_page_templates_use_shared_document_head_and_page_header(self):
        ...
        for template_name in page_templates:
            with self.subTest(template_name=template_name):
                ...
                self.assertNotIn('class="page-title"', source)
